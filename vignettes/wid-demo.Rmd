...
author: |
    | Author: Thomas Blanchet (updated by Ignacio Flores)
...
linestretch: 1.2
vignette: >
  %\VignetteIndexEntry{Download data from WID.world into R}
...
\texttt{a}               & average (local currency unit, last year’s prices)             \\
\texttt{b}               & inverted Pareto-Lorenz coefficient (no unit)                 \\
\texttt{f}               & female population (fraction, between 0 and 1)                \\
\texttt{g}               & Gini coefficient (between 0 and 1)                            \\
\texttt{i}               & index (no unit)                                              \\
\texttt{n}               & population (people)                                          \\
\texttt{s}               & share (fraction, between 0 and 1)                            \\
\texttt{t}               & threshold (local currency unit, last year’s prices)          \\
\texttt{m}               & total (local currency unit, last year’s prices)              \\
\texttt{p}               & proportion of women (fraction, between 0 and 1)             \\
\texttt{w}               & wealth-to-income ratio or labor/capital share (fraction of national income) \\
\texttt{r}               & Top 10/Bottom 50 ratio (no unit)                             \\
\texttt{x}               & exchange rate (market or PPP) (local currency unit per foreign currency) \\
\texttt{e}               & Total emissions (Tons of CO2 equivalent emissions)           \\
\texttt{k}               & Per capita emissions (Tons of CO2 equivalent emissions)      \\
\texttt{l}               & Average per capita group emissions (Tons of CO2 equivalent per capita emissions) \\ \bottomrule
\end{tabularx}
...
\texttt{999}          & all ages \\
\texttt{014}          & ages 0-14 \\
\texttt{156}          & ages 15-64 \\
\texttt{997}          & ages 65+ \\
\texttt{991}          & ages 0-19 \\
\texttt{992}          & ages 20+ (adults, including elderly) \\
\texttt{993}          & ages 20-39 \\
\texttt{994}          & ages 40-59 \\
\texttt{995}          & ages 60+ \\
\texttt{996}          & ages 20-64 (adults, excluding elderly) \\
\texttt{998}          & ages 80+ \\
\bottomrule
...
one-letter code & description \\ \midrule
\texttt{i}               & individuals \\
\texttt{j}               & equal-split adults (i.e., income or wealth divided equally among spouses) \\
\texttt{m}               & male \\
\texttt{f}               & female \\
\texttt{t}               & tax units \\
\texttt{e}               & employed \\ \bottomrule
...
) %>% dplyr::rename(ppp = value) %>% select(-year, -percentile)
